(function() {
    function onReady() {
        ...
        console.log('User has review:', window.userHasReview);
        console.log('User has ticket:', window.userHasTicket);
        console.log('Event match ID:', window.eventMatchId);
        console.log('Add modal element:', document.getElementById('addReviewModal'));
        console.log('Review container:', document.getElementById('review-container'));
        console.log('Review cards found:', document.querySelectorAll('[id^="review-"]').length);
    ...
    // === MODAL FUNCTIONS ===
    function initializeAddReviewButtons() {
        // Handle all buttons with onclick attribute for openAddModal
        document.querySelectorAll('button[onclick*="openAddModal"]').forEach(button => {
            const originalOnclick = button.getAttribute('onclick');
            button.removeAttribute('onclick');
            
            button.addEventListener('click', function() {
                console.log('Add review button clicked via event listener');
                openAddModal(window.eventMatchId);
            });
        });

        // Also handle button with specific ID if exists
    ...
    window.openAddModal = function(eventIdFromButton = null) {
        ...
        // If user has review, show toast dan return
        if (window.userHasReview === true || window.userHasReview === 'true') {
            console.log('User already has a review, showing toast');
            // ensure showToast exists
            if (typeof showToast === 'function') {
                showToast('You have already submitted a review for this event', 'error');
            } else {
        ...
        // If user has ticket, show toast and return
        if (window.userHasTicket === false || window.userHasTicket === 'false' || typeof window.userHasTicket === 'undefined') {
            console.log('User does not have ticket, blocking add modal');
            if (typeof showToast === 'function') {
                showToast('You need a confirmed ticket to add a review', 'error');
            } else {
                alert('You need a confirmed ticket to add a review');
            }
            return;
        }
    ...
    // Namespaced wrapper to avoid conflicts with other openEditModal functions on the page
    window.openEditModalReview = function(reviewId, rating, komentar) {
        if (typeof window.openEditModal === 'function') {
            window.openEditModal(reviewId, rating, komentar);
        } else {
